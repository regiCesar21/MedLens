...
def calculate_stats(chunk, stats_results):
    for column in chunk.columns:
        ...
        try:
            ...
            pass  # Ignore columns that cannot be somadas
...
def process_file(file_path, chunksize=10**5):
    ...
    with ThreadPoolExecutor(max_workers=12) as executor:
        for chunk in tqdm(pd.read_csv(file_path, chunksize=chunksize, low_memory=False), unit="chunk"):
...
# Filtrar dados de SpO2 (ITEMID = 220277) e definir a coluna TARGET
spo2 = sample_df[sample_df['ITEMID'] == 220277]
spo2['TARGET'] = spo2['VALUENUM'].apply(lambda x: 1 if x < 90 else 0)

# Dividir os dados em conjuntos de treinamento e teste
X = spo2.drop(['TARGET', 'VALUENUM'], axis=1)
y = spo2['TARGET']
...
for chunk in tqdm(pd.read_csv(DATA_PATH, chunksize=chunk_size, low_memory=False), unit='chunk'):
    spo2_chunk = chunk[chunk['ITEMID'] == 220277]
    spo2_chunk['TARGET'] = spo2_chunk['VALUENUM'].apply(lambda x: 1 if x < 90 else 0)
    X_chunk = spo2_chunk.drop(['TARGET', 'VALUENUM'], axis=1)
    y_chunk = spo2_chunk['TARGET']
